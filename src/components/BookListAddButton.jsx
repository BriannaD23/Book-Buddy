...
const BookListWithAddButton = ({ books, fetchBooks }) => {
  ...
  const handleAddBook = async (book) => {
    ...
    if (!token) {
      setError("You need to be logged in to add a book to your library.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Optionally fetch the updated library or just append the book to local state
        fetchUserLibrary(); // Fetch the updated library
      } else {
  ...
  const fetchUserLibrary = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(errorData.message || "Failed to fetch the library.");
      }
    } catch (error) {
      console.error(error);
      setError("An error occurred while fetching the library.");
    }
  };
